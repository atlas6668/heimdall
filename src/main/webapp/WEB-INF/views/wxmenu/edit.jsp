...
<body>
	...
	<ul class="nav nav-tabs">
		<li><a href="<spring:url value='/wxmenu/show'/>" data-toggle="tab">菜单</a></li>
		<li class="active"><a data-toggle="tab">编辑</a></li>
	</ul>
	
	<spring:url value="/wxmenu/save" var="save" />
	<form:form id="s_form" action="${save}" modelAttribute="model" method="post" 
		...
		<div class="control-group">
			...
			<div class="controls">
				<select name="fatherButton">
					<option value="">无</option>
					<c:forEach items="${firstlevelbuttons}" var="firstlevelbutton" varStatus="stat">
						<option value="${firstlevelbutton.id}"${model.fatherButton==firstlevelbutton.id?" selected='selected'":""}>${firstlevelbutton.name}</option>
					</c:forEach>
				</select>
			</div>
		</div>
